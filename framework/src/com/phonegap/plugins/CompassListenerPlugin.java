 ...
 */
public class CompassListenerPlugin extends Plugin implements SensorEventListener {
	 ...
	 * Constructor.
	 */
	public CompassListenerPlugin() {
		this.timeStamp = 0;
		this.setStatus(CompassListenerPlugin.STOPPED);
	}
	...
	private int start() {
		...
		if ((this.status == CompassListenerPlugin.RUNNING) || (this.status == CompassListenerPlugin.STARTING)) {
			return this.status;
		}
		...
		if (list.size() > 0) {
			...
			this.setStatus(CompassListenerPlugin.STARTING);
		}

		// If error, then set status to error
		else {
			this.setStatus(CompassListenerPlugin.ERROR_FAILED_TO_START);
		}
	 ...
	 */
	private void stop() {
		if (this.status != CompassListenerPlugin.STOPPED) {
	...
	public void onSensorChanged(SensorEvent event) {
		...
		this.setStatus(CompassListenerPlugin.RUNNING);
