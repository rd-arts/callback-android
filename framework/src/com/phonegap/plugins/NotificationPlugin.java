...
public class NotificationPlugin extends Plugin {
	...
	private final static String TAG = "GAP_" + NotificationPlugin.class.getSimpleName();
	...
	public NotificationPlugin() {
		//Log.d(TAG, "Construct NotificationPlugin");
	}
	...
	private synchronized void alert(final String message, final String title, final String buttonLabel, final String callbackId) {
		...
		final Activity ctx = this.ctx.getActivity();
		final NotificationPlugin notificationPlugin = this;
				...
				dlg.setPositiveButton(buttonLabel,
						new AlertDialog.OnClickListener() {
							public void onClick(DialogInterface dialog, int which) {
								dialog.dismiss();
								notificationPlugin.success(new PluginResult(PluginResult.Status.OK, 0), callbackId);
							}
						});
				dlg.create();
				dlg.show();
				...
				if (fButtons.length > 0) {
					dlg.setPositiveButton(fButtons[0],
							new AlertDialog.OnClickListener() {
								public void onClick(DialogInterface dialog, int which) {
									dialog.dismiss();
									notificationPlugin.success(new PluginResult(PluginResult.Status.OK, 1), callbackId);
								}
							});
				}
				...
				if (fButtons.length > 1) {
					dlg.setNeutralButton(fButtons[1],
							new AlertDialog.OnClickListener() {
								public void onClick(DialogInterface dialog, int which) {
									dialog.dismiss();
									notificationPlugin.success(new PluginResult(PluginResult.Status.OK, 2), callbackId);
								}
							});
				}
				...
				if (fButtons.length > 2) {
					dlg.setNegativeButton(fButtons[2],
							new AlertDialog.OnClickListener() {
								public void onClick(DialogInterface dialog, int which) {
									dialog.dismiss();
									notificationPlugin.success(new PluginResult(PluginResult.Status.OK, 3), callbackId);
								}
							}
					);
				}
	...
	private synchronized void activityStart(final String title, final String message) {
		...
		Runnable runnable = new Runnable() {
			public void run() {
				notificationPlugin.spinnerDialog = ProgressDialog.show(ctx, title, message, true, true,
	...
	private synchronized void progressStart(final String title, final String message) {
		...
		Runnable runnable = new Runnable() {
			public void run() {
				notificationPlugin.progressDialog = new ProgressDialog(ctx);
				notificationPlugin.progressDialog.setProgressStyle(ProgressDialog.STYLE_HORIZONTAL);
				notificationPlugin.progressDialog.setTitle(title);
				notificationPlugin.progressDialog.setMessage(message);
				...
				notificationPlugin.progressDialog.setOnCancelListener(
						new DialogInterface.OnCancelListener() {
							public void onCancel(DialogInterface dialog) {
